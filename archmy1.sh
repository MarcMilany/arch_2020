...
echo -e "${RED}=> ${YELLOW}ПРЕДУПРЕЖДЕНИЕ! ${BOLD}Следующая команда удалит все данные в зашифрованном разделе. Вы потеряете всю свою информацию! Поэтому убедитесь, что вы сделали резервную копию своих данных на внешнем источнике, таком как NAS или жесткий диск, прежде чем вводить любую из следующих команд. ${NC}"



В этом примере я буду использовать раздел с именем / dev / xvdc, и нашей первой задачей будет 3 раза перезаписать этот раздел случайными данными, этого достаточно, чтобы защитить вас от криминалистического расследования. Мне потребовалось около 30 минут, чтобы раздел размером 20 ГБ был перезаписан 3 раза.
